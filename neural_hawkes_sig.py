...
for datap in data_list:
    ...
    valid_result = open('./results/hawkes/results_rmsprop_sig.txt', 'a')
    tmp_f = open('./tmp_sig.txt', 'a')
    ...
    for e in range(20):
        ...
        if best_valid_f1 < valid_f1:
            ...
            for sig_idx in range(100):
                indices = list(range(len(y_tests)))
                np.random.seed(sig_idx)
                indices = np.random.choice(indices, size=len(indices))
                y_tmp = [y_tests[item] for item in indices]
                y_pred_tmp = [y_preds_test[item] for item in indices]
                tmp_f.write(str(f1_score(y_true=y_tmp, y_pred=y_pred_tmp, average='weighted'))+'\n')
                tmp_f.flush()
            tmp_f.write('\n')
