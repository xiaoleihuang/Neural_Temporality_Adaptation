'''Two inputs, domain prediction and class prediction inputs.

The method sample from both training and test data for domain prediction
while only sample training data fro the class prediction task
'''
...
def run_dnn(data_pair):
    ...
    if 'dianping' in data_pair[1] or 'amazon' in data_pair[1] or 'yelp' in data_pair[1]:
        ...
        model_sent.compile(
            ...
            loss_weights={'senti': 1, 'domain':0.001},
            optimizer='adam')
    else:
