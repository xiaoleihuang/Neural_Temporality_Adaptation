...
def load_data(filep, seed=-1):
    ...
    if seed != -1:
        ...
        data['x'] = np.asarray([data['x'][item] for item in indices])
        data['y'] = np.asarray([data['y'][item] for item in indices])
...
if __name__ == '__main__':
    ...
    # remove all save models
    removes = os.listdir('./models_sig/')
    for rme in removes:
        os.remove('./models_sig/'+rme)

    # loop through each data
    for datap in data_list:
        print('Working on: -----------------'+datap[0])
        valid_result = open('./results_cnn_sig.txt', 'a')
        ...
        for order, filen in enumerate(train_files):
            ...
            # compile the model
            prev_model = None
            if os.path.exists('./models_sig/'+datap[1]+'.h5'):
                prev_model = keras.models.load_model('./models_sig/'+datap[1]+'.h5')
            ...
            for e in range(20):
                ...
                if best_valid < valid_f1:
                    ...
                    model.save('./models_sig/'+datap[1]+'.h5')
                    best_model = model

            if order == len(train_files) - 1: # only use the last train domain
                '''test step'''
                print('---------------------test---------------------')
                tmp_f = open('tmp.txt', 'a')
                tmp_f.write(datap[1]+'\n')
                for sig_idx in range(100):
                    # load test data
                    test_data = load_data(dirp+datap[1]+'.test', seed=sig_idx)
                    ...
                    valid_result.write('\n')        
                    valid_result.flush()
                    tmp_f.write(str(f1_score(y_true=y_tests, y_pred=y_preds, average='weighted')))
                    tmp_f.write('\n')
                tmp_f.write('\n')
                tmp_f.flush()
                tmp_f.close()
