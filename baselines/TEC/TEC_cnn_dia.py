'''
Recurrent CNN mode with contextual information, using diachronic word embedding
'''
import os
import sys
...
def TEC_basic(config, wt_matrix, f_prev=None, m_prev=None):
    '''
...
if __name__ == '__main__':
    ...
    dia_list = ['hamilton'] # 'kim', 'kulkarni', 'hamilton' ## 'my', 

    for dia in dia_list:
        rfiles = os.listdir('./models_cnn'+mode+'/')
        for rfile in rfiles:
            os.remove('./models_cnn'+mode+'/'+rfile)

        for datap in data_list:
            print('Working on: -----------------'+datap[0])
            valid_result = open('./results_cnn_dia.txt', 'a')
            ...
            for order, filen in enumerate(train_files):
                # load the diachronic word embeddings
                wt_matrix = np.load('./wt'+'_'+dia+'/'+datap[0]+'/'+str(order+1)+'.npy')
                ...
                # compile the model
                prev_model = None
                if os.path.exists('./models_cnn'+mode+'/'+datap[1]+'.h5'):
                    prev_model = keras.models.load_model('./models_cnn'+mode+'/'+datap[1]+'.h5')
                    ...
                    model = TEC_basic(config, wt_matrix=wt_matrix, f_prev='')
                else:
                    model = TEC_basic(config, wt_matrix=wt_matrix,  f_prev=None)
                ...
                for e in range(15):
                    ...
                    if best_valid < valid_f1:
                        ...
                        model.save('./models_cnn'+mode+'/'+datap[1]+'.h5')
                        best_model = model
                ...
                if order == len(train_files) - 1: # only use the last train domain
                    ...
                    if config['pred_num'] == 3:
                        ...
                        y_preds = [np.round(item[0]) for item in y_preds]
                    # write results
                    valid_result.write(datap[1] + '\t' + tkey + '\t' + dia +'\n')
                    valid_result.write(
                        'Test result: ' +
